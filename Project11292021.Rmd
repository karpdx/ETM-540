...
MaxhoursPerClass <- c(200, 200, 400, 100)
print(MaxhoursPerClass[3])
...
add_variable(Vx[i,j,k], i=1:n, j=1:p, k=1:c, type= "continuous") %>%
  ...
  ##  hours needed for project i to perform task in classification k
    ...
    set_objective(sum_expr((Vy[i,j,k]*Billing[i,j]*0.1), i=1:n, j=1:p, k=1:c), "max") %>%
    ...
    add_constraint (sum_expr (Vx[i,j,k], i=1:n, j=1:p, k=1:c) <= MaxhoursPerClass[k], k=1:4)
```
