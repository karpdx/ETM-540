...
-   Skills set: Every employee $i$ assigned on any project $j$ should have the appropriate skill.
...
```{r}
BI <- as.matrix(Billingrate)
startt<-cbind(Billingrate, Classifications)

resss <- cbind(startt, matrix(cbind(t(get_solution(result,X[1,j,1])[,3]),
